...
function Create(self)
	...
	self.soundImpact = CreateSoundContainer("Bow Arrow Impact", "FGround.rte");
	
	self.soundWiggle = CreateSoundContainer("Bow Arrow Wiggle", "FGround.rte");
	self.soundWigglePlay = true
	
	self.soundBounce = CreateSoundContainer("Bow Arrow Bounce", "FGround.rte");
	...
	if self.phase == 0 and self.Vel.Magnitude > 13 then -- Raycast, stick to things
		...
		if moCheck ~= rte.NoMOID then
			...
			if IsMOSRotating(MO) then
				...
				local effect = CreateMOSRotating("Bow Arrow Hit Effect");
				if effect then
